...
class PerformanceBenchmark:
        ...
        for size_name, config in size_configs:
            ...
            for backend in backends:
                ...
                for run in range(num_runs):
                    ...
                    if result.success:
                        print(
                            f"    Run {run + 1}: Forward={result.forward_time_ms:.2f}ms, "
                ...
                if successful_runs:
                    averaged_result = self._average_results(
                        successful_runs, backend, config
                    )
                    all_results.append(averaged_result)

                    print(
                        f"    ✅ Average: Forward={averaged_result.forward_time_ms:.2f}ms, "
    ...
    def _benchmark_single_run(
        self, backend: str, config: dict[str, Any], run_number: int
    ) -> BenchmarkResult:
        ...
        try:
            ...
            sample_input = np.random.randint(
                1, vocab_size - 1, size=(batch_size, seq_len)
            )
            sample_targets = np.random.randint(
                1, vocab_size - 1, size=(batch_size, seq_len)
            )
    ...
    def _average_results(
        self, results: list[BenchmarkResult], backend: str, config: dict[str, Any]
    ) -> BenchmarkResult:
        ...
        backward_times = [
            r.backward_time_ms for r in results if r.backward_time_ms is not None
        ]
        avg_backward = (
        ...
        training_times = [
            r.training_time_ms for r in results if r.training_time_ms is not None
        ]
        avg_training = (
    ...
    def _compare_backends_original(
        ...
        for config_key, group_results in config_groups.items():
            ...
            for result in successful_results:
                ...
                efficiency = (
                    result.forward_time_ms / result.memory_usage_mb
                )  # ms per MB
    ...
    def generate_report(
        self, results: list[BenchmarkResult], save_path: str | None = None
    ) -> str:
        ...
        report.append(
            f"{'Model':<15} {'Backend':<8} {'Forward':<12} {'Memory':<10} {'Speedup':<8} {'Efficiency':<10}"
        )
        report.append("-" * 75)
        ...
        for config_name, data in analysis.items():
            ...
            for backend, metrics in data["backend_comparison"].items():
                ...
                report.append(
                    f"     • Forward time: {metrics['forward_time_ms']:.2f}ms"
                )
                report.append(
    ...
    def benchmark_backend(
        self, backend: str, config: dict[str, Any], num_runs: int = 1
    ) -> dict[str, float]:
    ...
    def compare_backends(
        ...
        if backends is not None and config is not None:
            try:
                ...
                results = self.benchmark_model_sizes(
                    [config], backends, num_runs=num_runs
                )
                ...
                for result in results:
                    comparison[result.backend] = {
                        "score": 100.0
                        - result.forward_time_ms,  # Simple scoring scheme
                        "execution_time": result.forward_time_ms,
                        "memory_usage": result.memory_usage_mb,
                        "parameter_count": result.parameter_count,
                    }
...
def main():
    ...
    print(
        f"   Results: {len([r for r in results if r.success])} successful, {len([r for r in results if not r.success])} failed"
    )
