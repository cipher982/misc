...
class PythonAttention(AbstractAttention):
    ...
    def forward(
        ...
        if self.verbose:
            print(
                f"[PythonAttention] Forward pass: batch_size={batch_size}, seq_len={seq_len}"
            )
        ...
        concat_output = self._concatenate_heads(
            attention_output
        )  # (batch, seq, hidden)
    ...
    def _compute_attention_scores(
        self, q: list[list[list[list[float]]]], k: list[list[list[list[float]]]]
    ) -> list[list[list[list[float]]]]:
        ...
        scores = [
            [
                [[0.0 for _ in range(seq_len)] for _ in range(seq_len)]
                ...
                for i in range(seq_len):  # Query position
                    for j in range(seq_len):  # Key position
                        ...
                        for dim in range(head_dim):
                            dot_product += (
                                q[batch][i][head][dim] * k[batch][j][head][dim]
                            )
    ...
    def _apply_mask(
        self, scores: list[list[list[list[float]]]], mask: Any
    ) -> list[list[list[list[float]]]]:
    ...
    def _apply_softmax(
        self, scores: list[list[list[list[float]]]]
    ) -> list[list[list[list[float]]]]:
