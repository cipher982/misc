...
def randn(shape: tuple[int, ...], scale: float = 0.02) -> list[Any]:
    ...
    if len(shape) == 1:
        ...
        return [[[random.gauss(0, scale) for _ in range(shape[2])]
...
def matmul_3d(a: list[list[list[float]]], b: list[list[float]]) -> list[list[list[float]]]:
        ...
        for seq in range(seq_len):
            for out_dim in range(dim2):
                for in_dim in range(len(b)):
                    result[batch][seq][out_dim] += a[batch][seq][in_dim] * b[in_dim][out_dim]
...
def add_tensors(a: list[Any], b: list[Any]) -> list[Any]:
    ...
    elif not isinstance(a[0][0], list):
        # 2D case
        return [[a[i][j] + b[i][j] for j in range(len(a[i]))] for i in range(len(a))]
    else:
        # 3D case
        return [[[a[i][j][k] + b[i][j][k] for k in range(len(a[i][j]))]
...
def relu(x: list[Any]) -> list[Any]:
    ...
    if not isinstance(x[0], list):
        ...
        # 3D case
        return [[[max(0.0, x[i][j][k]) for k in range(len(x[i][j]))]
...
def gelu(x: list[Any]) -> list[Any]:
    ...
    if not isinstance(x[0], list):
        ...
        # 3D case
        return [[[gelu_single(x[i][j][k]) for k in range(len(x[i][j]))]
                 for j in range(len(x[i]))] for i in range(len(x))]
...
def swish(x: list[Any]) -> list[Any]:
    ...
    if not isinstance(x[0], list):
        ...
        # 2D case
        return [[swish_single(x[i][j]) for j in range(len(x[i]))] for i in range(len(x))]
    else:
        # 3D case
        return [[[swish_single(x[i][j][k]) for k in range(len(x[i][j]))]
                 for j in range(len(x[i]))] for i in range(len(x))]


def layer_norm(x: list[list[list[float]]], eps: float = 1e-6) -> list[list[list[float]]]:
