...
class TorchLayerNorm(AbstractNormalization, nn.Module):
    ...
    def backward(
        ...
        gradients = {
            "weight": self.layer_norm.weight.grad
            if self.layer_norm.weight.grad is not None
            else torch.zeros_like(self.layer_norm.weight),
            "bias": self.layer_norm.bias.grad
            if self.layer_norm.bias.grad is not None
            else torch.zeros_like(self.layer_norm.bias),
        }
...
class TorchRMSNorm(AbstractNormalization, nn.Module):
    ...
    def backward(
        ...
        gradients = {
            "weight": self.weight.grad
            if self.weight.grad is not None
            else torch.zeros_like(self.weight),
        }
