...
def plot_loss_history(loss_history: List[float], title: str = "Training Loss") -> plt.Figure:
    ...
    if loss_history:
        ...
        if len(loss_history) > 10:
            ...
            ax.plot(range(window, len(loss_history) + 1), moving_avg, 'r--', 
                   linewidth=2, alpha=0.7, label=f'{window}-step moving average')
            ax.legend()
...
def plot_attention_heatmap(attention_weights: np.ndarray, title: str = "Attention Weights") -> plt.Figure:
...
def plot_layer_statistics(layer_stats: List[Dict], stat_name: str, title: str = None) -> plt.Figure:
...
def plot_activation_distribution(activation_values: np.ndarray, activation_type: str) -> plt.Figure:
...
def plot_comparison_chart(configs: List[Dict], metrics: List[str], title: str = "Model Comparison") -> plt.Figure:
...
def create_dashboard_plots(model_stats: Dict) -> Dict[str, plt.Figure]:
    ...
    if model_stats.get('layer_stats'):
        ...
        plots['attention_weights_mean'] = plot_layer_statistics(
            layer_stats, 'attention_weights_mean', 'Mean Attention Weights Across Layers'
        )
    ...
    if (model_stats.get('layer_stats') and 
        model_stats['layer_stats'][0].get('attention', {}).get('attention_weights') is not None):
        attention_weights = model_stats['layer_stats'][0]['attention']['attention_weights']
