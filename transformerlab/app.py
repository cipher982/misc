...
from .viz.plots import create_dashboard_plots, plot_loss_history, plot_attention_heatmap
...
def main():
    ...
    with st.sidebar:
        ...
        corpus_file = st.selectbox(
            ...
            help="Choose the training corpus"
        )
        ...
        with col1:
            if st.button("🔄 Initialize Model"):
                initialize_model(
                    corpus_file, hidden_dim, num_layers, num_heads, ff_dim,
                    norm_type, residual_type, activation_type, pos_encoding_type
                )
    ...
    if st.session_state.model is not None:
        ...
        with col1:
            st.metric("Model Size", f"{get_model_size(st.session_state.model):.1f}K params")
        ...
        with col2:
            if model_stats['layer_stats']:
                ...
                for i, layer_stat in enumerate(model_stats['layer_stats']):
                    layer_data.append({
                        'Layer': i,
                        'Attention Mean': layer_stat['attention']['attention_weights_mean'],
        ...
        st.info("👈 Use the sidebar to configure and initialize your transformer model!")
...
def initialize_model(corpus_file: str, hidden_dim: int, num_layers: int, num_heads: int, 
                    ff_dim: int, norm_type: str, residual_type: str, activation_type: str, 
    ...
    try:
        ...
        st.success(f"✅ Model initialized with {get_model_size(model):.1f}K parameters!")
...
def train_model(batch_size: int, seq_len: int, num_steps: int):
    ...
    for step in range(num_steps):
        ...
        status_text.text(f"Training step {step + 1}/{num_steps} - Loss: {stats['loss']:.4f}")
...
def generate_text(prompt: str, max_length: int = 100) -> str:
    ...
    # Generate
    generated_tokens = model.generate(prompt_array, max_length=max_length, temperature=0.8)
...
def save_experiment():
    ...
    st.success(f"✅ Experiment saved! Total experiments: {len(st.session_state.experiment_history)}")
