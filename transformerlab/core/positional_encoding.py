...
def sinusoidal_positional_encoding(seq_len: int, hidden_dim: int, max_len: int = 10000) -> np.ndarray:
    """
...
def rope_positional_encoding(x: np.ndarray, seq_len: int, head_dim: int, max_len: int = 10000) -> np.ndarray:
    """
...
def alibi_positional_encoding(seq_len: int, num_heads: int, head_dim: int) -> np.ndarray:
    """
...
class SinusoidalPE(PositionalEncoding):
    ...
    def forward(self, x: np.ndarray, seq_len: int = None) -> np.ndarray:
        ...
        if self.pe is None or self.pe.shape[0] < seq_len:
            self.pe = sinusoidal_positional_encoding(seq_len, self.hidden_dim, self.max_len)
...
class ALiBi(PositionalEncoding):
    ...
    def forward(self, x: np.ndarray, seq_len: int = None) -> np.ndarray:
        ...
        if self.bias is None or self.bias.shape[1] != seq_len:
            self.bias = alibi_positional_encoding(seq_len, self.num_heads, self.head_dim)
