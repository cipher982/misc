...
class FeedForward:
    ...
    def __init__(self, hidden_dim: int, ff_dim: int, activation_type: str = "ReLU", 
    ...
    def _get_ff_stats(self, x: np.ndarray, h: np.ndarray, h_activated: np.ndarray, 
                     output: np.ndarray, final_output: np.ndarray) -> Dict:
...
def get_residual_type(residual_type: str) -> str:
    ...
    if residual_type not in valid_types:
        raise ValueError(f"Invalid residual type: {residual_type}. Must be one of {valid_types}")
