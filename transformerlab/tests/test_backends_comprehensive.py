...
class TestBackendFunctionality:
    ...
    def test_generation(self, backend_name, small_config, sample_input_data):
        ...
        if backend_name == "torch":
            ...
            prompt = torch.tensor(
                [sample_input_data["prompt_tokens"]], dtype=torch.long
            )
        else:
...
class TestBackendConsistency:
    ...
    def test_output_similarity(self, small_config):
        ...
        for backend_name in ["python", "numpy"]:
            try:
                transformers[backend_name] = create_transformer(
                    backend_name, **small_config
                )
        ...
        losses = [
            output["loss"] for output in outputs.values() if output["loss"] is not None
        ]
        if len(losses) >= 2:
    ...
    def test_parameter_count_consistency(self):
        ...
        if len(param_counts) >= 2:
            ...
            assert all(count == counts[0] for count in counts), (
                f"Parameter counts differ: {param_counts}"
            )
